...
public interface ProductTemplateRepository extends JpaRepository<ProductTemplate, UUID> {
    ...
    @Query("SELECT DISTINCT pt FROM ProductTemplate pt LEFT JOIN FETCH pt.variants v WHERE pt.organizationId = :organizationId AND pt.deletedAt IS NULL AND (v.deletedAt IS NULL OR v IS NULL) ORDER BY pt.createdAt DESC")
