...
public interface StockLevelRepository extends JpaRepository<StockLevel, UUID> {
    ...
    @Query("SELECT sl FROM StockLevel sl WHERE sl.inventoryItem.location.storeId = :storeId AND sl.available <= sl.lowStockThreshold AND sl.inventoryItem.deletedAt IS NULL")
    List<StockLevel> findLowStockItemsByStoreId(@Param("storeId") UUID storeId);
