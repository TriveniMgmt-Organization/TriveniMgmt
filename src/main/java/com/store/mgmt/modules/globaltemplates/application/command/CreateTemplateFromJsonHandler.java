...
@Component
@Transactional
public class CreateTemplateFromJsonHandler implements CommandHandler<CreateTemplateFromJsonCommand, GlobalTemplateResponseDTO> {

    private static final Logger log = LoggerFactory.getLogger(CreateTemplateFromJsonHandler.class);

    private final GlobalTemplateManagementService templateManagementService;

    public CreateTemplateFromJsonHandler(GlobalTemplateManagementService templateManagementService) {
        this.templateManagementService = templateManagementService;
    }

    @Override
    public GlobalTemplateResponseDTO handle(CreateTemplateFromJsonCommand cmd) {
        log.info("Creating global template from JSON");

        GlobalTemplate template = templateManagementService.createTemplateFromJson(cmd.jsonData());

        log.info("Created global template from JSON: {}", template.getCode());
