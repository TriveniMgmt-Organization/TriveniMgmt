...
public class GetActiveTemplatesHandler implements QueryHandler<GetActiveTemplatesQuery, List<GlobalTemplateResponseDTO>> {

    private static final Logger log = LoggerFactory.getLogger(GetActiveTemplatesHandler.class);

    private final GlobalTemplateRepository repository;
    private final GlobalTemplateMapper mapper;

    public GetActiveTemplatesHandler(GlobalTemplateRepository repository, GlobalTemplateMapper mapper) {
        this.repository = repository;
        this.mapper = mapper;
    }

    @Override
    public List<GlobalTemplateResponseDTO> handle(GetActiveTemplatesQuery query) {
        log.debug("Getting active templates");

        List<GlobalTemplate> templates = repository.findAllActive();
