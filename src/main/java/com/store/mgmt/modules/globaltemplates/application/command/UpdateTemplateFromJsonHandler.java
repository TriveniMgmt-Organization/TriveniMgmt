...
@Component
@Transactional
public class UpdateTemplateFromJsonHandler implements CommandHandler<UpdateTemplateFromJsonCommand, GlobalTemplateResponseDTO> {

    private static final Logger log = LoggerFactory.getLogger(UpdateTemplateFromJsonHandler.class);

    private final GlobalTemplateManagementService templateManagementService;

    public UpdateTemplateFromJsonHandler(GlobalTemplateManagementService templateManagementService) {
        this.templateManagementService = templateManagementService;
    }

    @Override
    public GlobalTemplateResponseDTO handle(UpdateTemplateFromJsonCommand cmd) {
        log.info("Updating global template from JSON: {}", cmd.templateId());

        GlobalTemplate template = templateManagementService.updateTemplateFromJson(cmd.templateId(), cmd.jsonData());

        log.info("Updated global template from JSON: {}", template.getCode());
